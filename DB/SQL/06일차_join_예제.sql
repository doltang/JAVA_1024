...
join 
- 두 개 이상의 테이블을 합쳐서 검색
- 이유 : 알고 잇는 데이터와 조회하는 데이터가 서로 다른 테이블에 있고,
	테이블들을 속성으로 연결할 수 있는 경우 사용
...
select 속성명들 from 테이블1
    ...
    [order by 절]
    [limit 절]
- 속성명1과 속성명2가 이름이 같으면 반드시 테이블명을 써줘야 한다.
- 속성명1과 속성명2가 이름이 다르면 테이블명을 생략할 수 있음.

-- 테이블 1과 테이블2가 연결, 테이블2와 테이블3이 연결
select 속성명들 from 테이블1
	join 테이블2
		on 테이블1.속성명1 = 테이블2.속성명2
	join 테이블3
...
*/
-- 제품명과 옵션명을 함께 조회
select * from product; -- pr_title
select * from product_option; -- _po_title
...
select distinct mc_title as 분류, pr_title as 제품명, 
po_title as 옵션 from product
...
-- 분류별 등록된 제품명을 조회
select distinct mc_title as 분류, sc_title as 소분류, 
ifnull(pr_title, '등록된 제품 없음') from product 
