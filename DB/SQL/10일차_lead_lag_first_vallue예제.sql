...
- lead(속성, n, x) : 속성의 다음 n개의 값을 가졍고 없으면 x로 대체
- lag(속성) : 속성의 이전 값을 가져옴
- lag(속성, n, x) : 속성의 이전 n개의 값을 가졍고 없으면 x로 대체
...
select
	...
	pr_title, pr_price, 
    pr_price - lead(pr_price,1,0) over(order by pr_price desc) 
    as 가격차이 from product;
    
select
	row_number() over(order by pr_price desc)
	pr_title, pr_price, 
    pr_price - lag(pr_price,1,pr_price) over(order by pr_price desc) 
    as 가격차이 from product;
