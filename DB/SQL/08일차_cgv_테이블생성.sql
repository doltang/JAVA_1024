...
CREATE TABLE `movie_person` (
	`mp_num`	int auto_increment primary key,
	`mp_name`	varchar(50),
	`mp_birth`	date,
	`mp_contry`	varchar(50)	
);
...
CREATE TABLE `movie_genre` (
	`mg_num`	int auto_increment primary key,
...
CREATE TABLE `trailer` (
	`tr_num`	int auto_increment primary key,
	`tr_title`	varchar(50),
	`tr_file_name`	varchar(100),
...
CREATE TABLE `theater` (
	`th_num`	int auto_increment primary key,
	`th_name`	varchar(20),
	`th_lot_address`	varchar(50),
	`th_address`	varchar(50),
	`th_trans_info`	longtext,
	`th_parking_info`	longtext,
	`th_number`	varchar(10),
	`th_total_cinema`	int	not NULL,
...
CREATE TABLE `seat` (
	`se_num`	int auto_increment primary key,
	`se_name`	varchar(10),
	`se_state`	varchar(10),
	`se_ci_num`	int NOT NULL,
	`se_price`	int	not null
);

DROP TABLE IF EXISTS `screen_schedule`;

CREATE TABLE `screen_schedule` (
	`ss_num`	int auto_increment primary key,
	`ss_date`	date,
	`ss_time`	time,
	`ss_total_seat`	int not null,
	`ss_possible_seat`	int	not null,
...
CREATE TABLE `member` (
	...
	`me_pw`	varchar(50)	,
	`me_movie_count`int not null
);

DROP TABLE IF EXISTS `ticketing`;

CREATE TABLE `ticketing` (
	`ti_num`	int auto_increment primary key,
	`ti_amount`	int	not null,
	`ti_me_id`	varchar(20),
...
CREATE TABLE `ticketing_seat` (
	`ts_num`	int auto_increment primary key,
