...
select : 디비 사용자가 컴퓨터 DB에 데이터를 조회하기 위해 사용하는 언어
update : 디비 사용자가 컴퓨터 DB에 데이터를 수정하기 위해 사용하는 언어
delete : 디비 사용자가 컴퓨터 DB에 데이터를 삭제하기 위해 사용하는 언어
...
int(4) : (n)은 zf 옵션을 선택하면 1을 저장할 때 0001로 저장
		 zf 옵션을 선택안하면 일반 int와 동일

슈퍼키 : 튜플을 유일하게 식별하게 하는 속성들의 집합 (포괄적, 쓸모없는 것들도 포함되있음)
후보키 : 슈퍼키에서 불필요한 속성들을 제거한 키 
	=> 최소한의 속성으로 튜플을 유일하게 식별하게 하는 속성들의 집합
기본키 : 후보키에서 선정된 키 => 의미상 적절해야 함
대리키 : 테이블에 기본키가 복합 속성으로 되어 있는 경우 
	일련번호로 대체하는 속성을 기본키로 선정 (주문번호 등)
...
*/
select pr_title, if(pr_price < 20000, '가격 쌈', '비쌈') 
as '가격' from product;
select pr_title, 
if(pr_price < 20000, '가격 쌈', if(pr_price < 3000, '적당', '비쌈')) 
as '가격' from product;
select pr_title,
	case
		when pr_price < 20000
